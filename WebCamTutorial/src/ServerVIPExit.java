...
/**
 * Necessary database update for exit of VIP User; Both for 'Prepaid' and
 * 'Postpaid'
 */
public class ServerVIPExit {
	...
	public String vipExit(String rfid) {
		...
		PreparedStatement stmtEmployee, stmtqueryTime, stmtfareCalculate, stmtupdatePostpaid, stmtupdatePrepaid,
				stmtupdateRfidInfo, stmtReturn;
		try {
				...
				if (rsqueryTime.next() && isOk == 1) {
					...
					if (rsfareCalculate.next()) {
						...
						if (paymentType.equalsIgnoreCase("Prepaid") && remaintingBalance < totalFare) {
							...
							updatePostpaid = "UPDATE `vip_rfid` SET " + "`VIPrfid_balance_used`=?,"
							...
							returnResult = timeExit + "s" + usedBalance + "," + totalFare;
							...
							updatePrepaid = "UPDATE `vip_rfid` SET `VIPrfid_remaining_balance`=?,"
									+ "`VIPrfid_balance_used`=?," + "`isParked`=' 0 '  WHERE `generateRFID_NO`=? ";
							...
							// returnResult=timeExit+"s";
							returnResult = timeExit + "s" + remaintingBalance + "," + usedBalance + "," + totalFare;
						}
						if (isupdate == 1) {
							updateRfidInfo = "UPDATE `vip_rfid_info` SET `rfid_info_status`=' 0 ',`exit_time`=now() , `exit_booth_id`=?,`balance`=? "
									+ " WHERE `rfid_no`=? " + "AND `rfid_info_status`=' 1 ' ";
							stmtupdateRfidInfo = MyConnection.getConnection().prepareStatement(updateRfidInfo);
							stmtupdateRfidInfo.setString(1, exitBooth);
							stmtupdateRfidInfo.setDouble(2, totalFare);
							stmtupdateRfidInfo.setString(3, cardno);
