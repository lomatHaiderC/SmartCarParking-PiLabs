	...
	public static void dbConnection() throws IOException {
		...
		try {
			...
			while (rs.next()) {
				...
				String queryImage = "";
				if (option == 2) {
					queryImage = "UPDATE `rfid_info` SET `image`=? WHERE `rfid_no`='" + lastName
							+ "' and  `image` is NULL limit 1";
				} else if (option == 1) {
					queryImage = "UPDATE `vip_rfid_info` SET `image`=? WHERE `rfid_no`='" + lastName
							+ "' and  `image` is NULL limit 1";
				}
				...
				// File f=new File("H:\\\\\\\\Study\\\\\\\\DBMS LAB\\\\\\\\Car Parking Project
				// Data\\\\\\\\project\\\\\\\\Socket\\\\\\\\WebCamTutorial\\\\\\\\"+name);
