...
/**
 * Database Connection when user forgets his password It will provide new
 * password '123456' for Employee and provide '123456789' for Admin
 */
		...
		try {
			...
			if (rsReadEmployee.next()) {
				...
				if (ques.contains(inputQues)) {
					...
					queryUpdate = " UPDATE `employ` SET `employ_pass`='" + 123456 + "' " + "WHERE `employ_mail`='"
							+ splitData[1] + "' ";
				...
				if (ques.contains(inputQues)) {
					...
					queryUpdate = " UPDATE `admin` SET `admin_pass`='" + 123456789 + "' " + "WHERE `admin_mail`='"
							+ splitData[1] + "' ";
					stmtUpdate = MyConnection.getConnection().prepareStatement(queryUpdate);
					stmtUpdate.executeUpdate();
